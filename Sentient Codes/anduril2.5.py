...
#HORIZONs base data
AZ_veenus_HOR = 75.806271
EL_veenus_HOR = 69.278559
AZ_maa_HOR =    94.647066
EL_maa_HOR =    66.507475
AZ_paike_HOR =  89.324328
EL_paike_HOR =  13.697882

HOR_veenus   = angle_to_dir(EL_veenus_HOR, AZ_veenus_HOR)
HOR_maa      = angle_to_dir(EL_maa_HOR, AZ_maa_HOR)
HOR_paike    = angle_to_dir(EL_paike_HOR, AZ_paike_HOR)
HOR_keskmine = (HOR_veenus + HOR_maa + HOR_paike)/3   #pole ühikvektor
ro = np.linalg.norm(HOR_keskmine)
HOR_final_kesk = HOR_keskmine/ro
print("Keskmine Horizons suunavektor", HOR_final_kesk)
...
# Set initial vectors at correct angles for will_turner
...
# Final method for converting camera direction vectors into AZ and EL
...
#print(angle_to_dir(75.800407, 9.286656))
...
#print("\nnew transformed direction vector by Bill Turner\n",init_dataANG[0:5])
...
# The holy 5-part-step-vector-turning algorithm is applied here
molder = []
for i in np.arange(0, len(andmed), 1):
...
#print('max z =', max_z)    #kaamera 9311 kõrgus oli vale
                            #nüüd parandatud :)
# Ma täpselt ei taju mida zip(*data) teeb,         # best guess: 6-dim vector, mis nõuab
# kuid see vajalik quiver'i funksioneerimiseks     # list of arrays oma algandmeteks
...
flag_dat = flag_corCOORD.to_numpy()     # Tegelikult on ebavajalik samm
                                        # võib kasutada lic flag_shifter
X_flag, Y_flag, Z_flag = zip(*flag_dat) # NB! Ehk zip(*flag_shifter) töötab ka
...
X_SWC, Y_SWC, Z_SWC = zip(*SWC_dat) # sobib ka zip(*SWC_shifter)
...
X_Sband1, Y_Sband1, Z_Sband1 = zip(*Sband1_dat) # sobib ka zip(*Sband1_shifter)
...
X_Sband2, Y_Sband2, Z_Sband2 = zip(*Sband2_dat) # sobib ka zip(*Sband2_shifter)
...
X_ladder, Y_ladder, Z_ladder = zip(*ladder_dat) # sobib ka zip(*ladder_shifter)
...
ax.set_xlabel('X-telg (m) <---')
ax.set_ylabel('Y-telg (m) <---')
ax.set_zlabel('Z-telg (m) --->')
...
df_new.to_csv(r'FINAL_CALC.csv',
             index = False, header=True)
df_setupANG.to_csv(r'0_0_1.csv',index = False, header=True)
